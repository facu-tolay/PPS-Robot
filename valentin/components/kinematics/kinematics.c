...
void calculo_compensacion_rotacional(float velocidades_lineales_reales[VELOCITY_VECTOR_SIZE])
{
    ...
    {
        velocity_dependent_factor = 4.3 * velocidades_lineales_reales[1];
...
{
    float delta_x = fabs(current_position[0] - (desired_setpoint+0.1));
    float delta_y = fabs(current_position[1] - (desired_setpoint+0.1));
